...
#import "DRCameraController.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import "DRContextManager.h"
#import "sys/sysctl.h"
...
@interface DRCameraController () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate,AVCaptureMetadataOutputObjectsDelegate>
{
    BOOL bFirstTime;
    BOOL bNotLessThan5s;
}
...
@implementation DRCameraController
...
- (BOOL)setupSession:(NSError **)error {
    ...
    self.eaglContext = [DRContextManager sharedInstance].eaglContext;
    self.ciContext = [DRContextManager sharedInstance].ciContext;
...
- (void)startRecording {
	dispatch_async([self globalQueue], ^{
	    if (![self isRecording]) {
            ...
            self.assetWriterVideoInput.transform = DRTransformForDeviceOrientation(orientation);
...
//            DrCardInfo * drInfo = [self doRec:imageBuffer];
//            if (drInfo !=nil )
//            {
//                [self.recDelegate DRCardRecognited:drInfo];
//            }
        }
    }
}
...
CGAffineTransform DRTransformForDeviceOrientation(UIDeviceOrientation orientation) {
...
CGRect getDRGuideFrame( CGRect rect)
{
...
CGRect combinRectDR(CGRect A, CGRect B)
{
    CGFloat t,b,l,r;
...
-(DrCardInfo * )doRec:(CVImageBufferRef)imageBuffer
{
    @synchronized(self)
    {
        ...
        if(CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess)
        {
            ...
            int ret = EXDRCardRecoNV12ST(pixelAddress, cbCrBuffer, (int)width, (int)height, 1, &exdrCard);
            
            if (ret <= 0){
                NSLog(@"DRCardRecApi，ret[%d]", ret);
            }else{
                bHasResult = YES;
                NSLog(@"ret=[%d]", ret);
