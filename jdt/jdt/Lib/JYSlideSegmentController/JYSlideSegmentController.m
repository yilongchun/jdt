...
#define DEFAULT_TITLE_COLOR [UIColor colorWithRed:102/255. green:102/255. blue:102/255. alpha:1]
...
//    [self scrollToViewWithIndex:2 animated:NO];
}
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  segmentBarItem.titleLabel.text = vc.title;
    if (indexPath.row == _selectedIndex) {
        segmentBarItem.titleLabel.textColor = self.indicatorColor;
    }else{
        segmentBarItem.titleLabel.textColor = DEFAULT_TITLE_COLOR;
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  if (scrollView == self.slideView) {
    ...
    self.indicatorBgView.frame = frame;
      ...
      int index = floor((scrollView.contentOffset.x - Main_Screen_Width / 2) / Main_Screen_Width) + 1;
//      DLog(@"%d",index);
